...
def add(id:int, name:str, description:str, image_url:str, small_image_url:str, lookup_url:str):
    """ Add a token card to the database.
    ...
    try:
        image_name = f"{id}.jpg"
        image_path = f"assets/card_images/tokens/{image_name}"
        small_image_path = f"assets/small_card_images/tokens/{image_name}"
        ...
        get_db().execute(
            ...
            (
                id,
                get_card_type_id_by_name("Token"),
                name, description,
                image_path, small_image_path, lookup_url
            )
        )
    except Exception as e:
        print(f"Error adding token card: {e}")